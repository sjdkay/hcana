...
Int_t THcScintillatorPlane::ProcessHits(TClonesArray* rawhits, Int_t nexthit)
{
  ...
  while(ihit < nrawhits) {
	...
	if(fTofUsingInvAdc) {
	  ...
	  printf("debug tw %.2f %.2f %.2f %.2f %.2f %.2f\n",
	    tdc_pos*fScinTdcToTime,
	    fHodoPosInvAdcOffset[index],
	    fHodoPosInvAdcAdc[index]/TMath::Sqrt(TMath::Max(20.0*.020,adcint_pos)),
	    tdc_neg*fScinTdcToTime,
	    fHodoNegInvAdcOffset[index],
		 fHodoNegInvAdcAdc[index]/TMath::Sqrt(TMath::Max(20.0*.020,adcint_neg))) ;
	  ...
	  //	  printf("d %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f %6.1f \n",
	  //	 tdc_pos*fScinTdcToTime, tw_corr_pos ,fHodo_LCoeff[index],
	  //	 tdc_neg*fScinTdcToTime, tw_corr_neg, 2*fHodoCableFit[index],
	  //     fHodo_LCoeff[index]);
	}
